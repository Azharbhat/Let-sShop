...
const Shop = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/api/items")
            .then(res => {
                setMenItems(res.data.filter((item) => item.category.toLowerCase() === "men"))
                setKidsItems(res.data.filter((item) => item.category.toLowerCase() === "kids" ))
                setWomenItems(res.data.filter((item) => item.category.toLowerCase() === "women"))
                setLoading(false)
            })
